...
const fichiers = [
  ...
  'src/data/items.js', 'src/data/lore.js', 'src/data/events_written.js',
];
...
const {
  BESTIARY_FULL, PORTRAITS, LOCATIONS, EVENTS, CONTRACTS,
  ITEM_POOL, EVENTS_WRITTEN, TREE, TREE_ELFES, COMPANIONS_POOL, LOC_COORDS,
} = vm.runInContext(`({
  BESTIARY_FULL, PORTRAITS, LOCATIONS, EVENTS, CONTRACTS,
  ITEM_POOL, EVENTS_WRITTEN, TREE, TREE_ELFES, COMPANIONS_POOL, LOC_COORDS
})`, ctx);
...
/* ---- Événements écrits ---- */
let nbScenes = 0, nbChoix = 0;
for(const ev of EVENTS_WRITTEN){
  const nom = ev.id;
...
console.log(`Événements  ${EVENTS_WRITTEN.length} écrits (${nbScenes} scènes, ${nbChoix} choix) + ${EVENTS.length} générés`);
console.log(`Pouvoirs    ${powerIds.size} · Objets ${itemIds.size} · Portraits ${portrIds.size}`);
console.log('');
