...
/* Remettre un fichier au joueur, par le moyen qui marche là où il joue.
 * Une page hébergée n'a pas le droit de déclencher un téléchargement toute
 * seule : elle doit passer par son hôte, qui demande confirmation. Sur un
 * fichier local, c'est l'inverse. On essaie l'un, puis l'autre — et de toute
 * façon le texte de la sauvegarde reste affiché en dessous, ce qui est le seul
 * moyen qui fonctionne partout. */
async function proposerFichier(nom, texte){
  try{
    const hote = (typeof claude !== 'undefined' && typeof claude.use === 'function')
      ? await claude.use('downloads') : null;
    if(hote){ await hote.save({ filename: nom, data: texte }); return; }
  } catch(e){ return; }   // le joueur a refusé : ne pas insister par un second moyen
  try{
...
document.getElementById('btnExportSave').onclick = () => {
  ...
  proposerFichier('parias_vardhen_save.json', texte);
